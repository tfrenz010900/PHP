# Copyright 2018, 2019 Looking Glass Applications, LLC, All Rights Reserved.
#  Author:  Tim Crosno
...
#
#
#  CHANGE LOG (STARTED 9/11/19 HTC).
#************************** ALSO SET THE ACTUAL VARIABLE BELOW AROUND LINE 75 ******************************************************
# 9/11/19 htc - VERSION 2.02 - changed display times for Version and SM Machine ID to 2 secs each.
# 9/14/19 htc - VERSION 2.03 - Per TRF's request: changed display times for Version and SM Machine ID to 1 secs each.
# 9/19/19 htc - VERSION 2.04 - added pitch time-out more detailed info to log file, fixed bug says "TIMEOUT" in marguee when Pract Mode completes - now = "FINISHED".
# 9/20/19 htc - VERSION 2.05 - commented out one line throwning a "null" value error trying to write to log file.  to find search:  "row[7]"

#================================================================================================================================================
### 10/07/19 htc - VERSION 2.06 - ADDED - Sound block of code commented out and made TWO versions - "NS"=no sound, "WS"=WITH sound. we have to find
#                                  a better way to control sound - on/off, set audio clip.
###    the line right below here is WITH SOUND, the line after SKIPS SOUND Every time!  The ONLY diffs between "NS" and "WS" versions.
### THIS CHANGED/LINE COUMMENT IN/OUT is around line 3,100 of this program.
###    if CurrPitch_Call != 'SUCCESS':
###    if CurrPitch_Call != CurrPitch_Call:   #9/30/19 htc forced skip of sound for now - too long of delay in real use (tim Heckler - HIT Indoor)
#===================================================================================================================================================

# 10/03/19 htc - VERSION 2.07 has "freeze" fix in it - have to put version 2.06 fixes in here and make it "Master Copy"
#                          #10/3/19 htc - added "fix" to above line - check strXcoord 102 or 101- means no results from Ozan's Impact program. around line 4816
#
#
#
#10/08/19 htc created temp fixit version: 'VER-2.07wsForecMinus2Vertical'
#
#10/18/19 htc - took "TRF" values location x-ref tables out, then copied back ORIGINAL CODE- THIS CODE !!!   htc
#
#10/27/19 htc - HARD CODED +2+2 fix in this one.  Verson 2.08.
#
#
...
# 9/14/19 htc smMainVersion = 'VER-2.02'
...
#8/5/19 htc added "test" LHB and RHB to all modes.
global gbolDisplayLHB
global gbolDisplayRHB
...
#   print ('MainLog.txt file size:' + str(statinfo.st_size))
   if statinfo.st_size > 100000:
       # os.remove(MainLogFile)                          9/9/19 htc, added open(filename, mode, buffering =0) "Buffering =0" to open commands below (3)

#       MainLogFileObject = open(MainLogFileName, 'w', Buffering = 0)
    ...
    MainLogFileObject = open(MainLogFileName, 'w')
#    MainLogFileObject.buffer = 0
#    MainLogFileObject.line_buffering = False
...
# NOT USED 9/15/18 htc    gdblStart1 = 0
    ...
    if bolUseCustom2:  # 10/24/18 htc - TRF at 2228 threw for hr or two and came up with these.
        ...
        gdblStart24 = 80
        gdblStart25 = 85
        gdblStart26 = 101    #8/12/19 htc, changed this from 100 to 101 just in case we do get a 100 'percent' value from Ozans locator program.
                            # TRF called today and said during a recent outing/event, the system would "hang" on "top right" pitches.
                            # thought this MIGHT be a possible cause.
...
# 10/9/19 htc setup custom table to fix "Toms +2 bug -horizontally "X" axis
 ...
 #  bolUseCustom2 = True
    ...
    if bolUseTRF:  # 10/08/19 htc
        ...
        gdblStartB = 0   #custom 2 value 0
        gdblStartC = 0  #custom 2 value 5
        gdblStartD = 15  #custom 2 value 10
        gdblStartE = 18  #custom 2 value 12
        gdblStartF = 21  #custom 2 value 15
        gdblStartG = 26  #custom 2 value 18
        gdblStartH = 32  #custom 2 value 21
        gdblStartI = 38  #custom 2 value 26
        gdblStartJ = 42  #custom 2 value 32
        gdblStartK = 45  #custom 2 value 38
        gdblStartL = 49  #custom 2 value 42
        gdblStartM = 52  #custom 2 value 45
        gdblStartN = 55  #custom 2 value 49
        gdblStartO = 59  #custom 2 value 52
        gdblStartP = 62  #custom 2 value 55
        gdblStartQ = 68  #custom 2 value 58
        gdblStartR = 70  #custom 2 value 62
        gdblStartS = 74  #custom 2 value 68
        gdblStartT = 77  #custom 2 value 70
        gdblStartU = 82  #custom 2 value 74
        gdblStartV = 85  #custom 2 value 77
        gdblStartW = 88  #custom 2 value 82
        gdblStartX = 92  #custom 2 value 84
...
#9/9/19 htc added some "flush" commands to get the log files to dump/write to DISK if at all possible.  Also changed "Buffers=0" on log file open command.
    MainLogFileObject.flush()
#    MainLogFileObject.
#    file.flush()
...
#8/12/19 htc added global values for "hit" area in practice mode.  Needs to pass all the way up to EasyInsight.  Hit/miss still on COLOR (Green).
    global CurrentTargetHitAreaX1str
    global CurrentTargetHitAreaX1int
    global CurrentTargetHitAreaY1int
    global CurrentTargetHitAreaX2str
    global CurrentTargetHitAreaX2int
    global CurrentTargetHitAreaY2int

    CurrentTargetHitAreaX1str = ''
    CurrentTargetHitAreaX1int = 0
    CurrentTargetHitAreaY1int = 0
    CurrentTargetHitAreaX2str = ''
    CurrentTargetHitAreaX2int = 0
    CurrentTargetHitAreaY2int = 0


    if ThisSess_Type.upper() == 'PRACTICE':
        if strX1 >= 'A' and strX1 <= 'Z':
            CurrentTargetHitAreaX1str = strX1
            CurrentTargetHitAreaX1int = strX2int(strX1)
            if intY1 > 0 and intY1 < 27:
                CurrentTargetHitAreaY1int = intY1
                if strX2 >= 'A' and strX2 <= 'Z':
                    CurrentTargetHitAreaX2str = strX2
                    CurrentTargetHitAreaX2int = strX2int(strX2)
                    if intY2 > 0 and intY2 < 27:
                        CurrentTargetHitAreaY2int = intY2
...
#8/5/19 htc added RHB(RIGHT hand batter outline and LHB - LEFT)
    global gbolDisplayLHB
    global gbolDisplayRHB
    gbolDisplayLHB = False
    gbolDisplayRHB = False
    tmpTestString = CurrentTargetName.upper()
    txtLHBuc = '-LHB'
    txtRHBuc = '-RHB'

    if tmpTestString.find(txtLHBuc) > 0:
        gbolDisplayLHB = True
    if tmpTestString.find(txtRHBuc) > 0:
        gbolDisplayRHB = True

#8/5/19 htc - next lines for testing only.
#    tmpTestString = 'asldfkj-LHB'
#    if tmpTestString.find(txtLHBuc) > 0:
#        gbolDisplayLHB = True
#
#    tmpTestString = 'asldfkj-RHB'
#    if tmpTestString.find(txtRHBuc) > 0:
#        gbolDisplayRHB = True
...
#    gbolDisplayRHB = True
    if gbolDisplayRHB:
        ...
        PaintThisBox(SM_Red, 23, 3, 28, 3)
        PaintThisBox(SM_Yellow, 25, 4, 26, 4)
        PaintThisBox(SM_Red, 27, 4, 28, 4)
        PaintThisBox(SM_Yellow, 25, 5, 26, 5)
        PaintThisBox(SM_Red, 27, 5, 28, 5)
        PaintThisBox(SM_Yellow, 25, 6, 25, 6)
        PaintThisBox(SM_Red, 26, 6, 28, 6)
        PaintThisBox(SM_Red, 25, 7, 25, 7)
        PaintThisBox(SM_Yellow, 26, 7, 26, 7)
        PaintThisBox(SM_Red, 24, 8, 24, 8)
        PaintThisBox(SM_Blue, 25, 8, 28, 8)
        PaintThisBox(SM_Yellow, 25, 9, 25, 9)
        PaintThisBox(SM_Blue, 26, 9, 28, 9)
        PaintThisBox(SM_Yellow, 24, 10, 25, 10)
        PaintThisBox(SM_Blue, 26, 10, 28, 10)
        PaintThisBox(SM_Yellow, 24, 11, 24, 11)
        PaintThisBox(SM_Blue, 25, 11, 28, 11)
        PaintThisBox(SM_Blue, 26, 12, 28, 12)
        PaintThisBox(SM_Blue, 26, 13, 28, 13)
        PaintThisBox(SM_Blue, 26, 14, 28, 14)
        PaintThisBox(SM_Red, 26, 15, 28, 15)
        PaintThisBox(SM_Blue, 26, 16, 28, 16)
        PaintThisBox(SM_Blue, 26, 17, 28, 17)
        PaintThisBox(SM_Blue, 26, 18, 28, 18)
        PaintThisBox(SM_Blue, 26, 19, 28, 19)
        PaintThisBox(SM_Blue, 26, 20, 28, 20)
        PaintThisBox(SM_Blue, 27, 21, 28, 21)
        PaintThisBox(SM_Blue, 27, 22, 28, 22)
        PaintThisBox(SM_Yellow, 27, 23, 28, 23)
        PaintThisBox(SM_Yellow, 27, 24, 28, 24)
        PaintThisBox(SM_Yellow, 26, 25, 28, 25)
        PaintThisBox(SM_Yellow, 25, 26, 28, 26)


#    gbolDisplayLHB = True
#    gbolDisplayLHB = False
    if gbolDisplayLHB:
        PaintThisBox(SM_Red, 3, 1, 5, 1)
        PaintThisBox(SM_Red, 3, 2, 5, 2)
        PaintThisBox(SM_Red, 3, 3, 8, 3)
        PaintThisBox(SM_Yellow, 5, 4, 6, 4)
        PaintThisBox(SM_Red, 3, 4, 4, 4)
        PaintThisBox(SM_Yellow, 5, 5, 6, 5)
        PaintThisBox(SM_Red, 3, 5, 4, 5)
        PaintThisBox(SM_Yellow, 6, 6, 6, 6)
        PaintThisBox(SM_Red, 3, 6, 5, 6)
        PaintThisBox(SM_Red, 5, 7, 5, 7)
        PaintThisBox(SM_Yellow, 6, 7, 6, 7)
        PaintThisBox(SM_Red, 7, 8, 7, 8)
        PaintThisBox(SM_Blue, 3, 8, 6, 8)
        PaintThisBox(SM_Yellow, 6, 9, 6, 9)
        PaintThisBox(SM_Blue, 3, 9, 5, 9)
        PaintThisBox(SM_Yellow, 6, 10, 7, 10)
        PaintThisBox(SM_Blue, 3, 10, 5, 10)
        PaintThisBox(SM_Yellow, 7, 11, 7, 11)
        PaintThisBox(SM_Blue, 3, 11, 6, 11)
        PaintThisBox(SM_Blue, 3, 12, 5, 12)
        PaintThisBox(SM_Blue, 3, 13, 5, 13)
        PaintThisBox(SM_Blue, 3, 14, 5, 14)
        PaintThisBox(SM_Red, 3, 15, 5, 15)
        PaintThisBox(SM_Blue, 3, 16, 5, 16)
        PaintThisBox(SM_Blue, 3, 17, 5, 17)
        PaintThisBox(SM_Blue, 3, 18, 5, 18)
        PaintThisBox(SM_Blue, 3, 19, 5, 19)
        PaintThisBox(SM_Blue, 3, 20, 5, 20)
        PaintThisBox(SM_Blue, 3, 21, 4, 21)
        PaintThisBox(SM_Blue, 3, 22, 4, 22)
        PaintThisBox(SM_Yellow, 3, 23, 4, 23)
        PaintThisBox(SM_Yellow, 3, 24, 4, 24)
        PaintThisBox(SM_Yellow, 3, 25, 5, 25)
        PaintThisBox(SM_Yellow, 3, 26, 6, 26)
...
#8/3/18 htc - now Find the target def for this session that was passed to this routine. (should have read from SESSION RECORD).
    if TargetToDefine == 'SMV1-EARLYDEV' or TargetToDefine == 'EarlyDev' or TargetToDefine == 'EarlyDevelopment' or TargetToDefine == 'Early Development' or \
       TargetToDefine == 'SMV1-EARLYDEV-RHB' or TargetToDefine == 'EarlyDev-RHB' or TargetToDefine == 'EarlyDevelopment-RHB' or TargetToDefine == 'Early Development-RHB' or \
       TargetToDefine == 'SMV1-EARLYDEV-LHB' or TargetToDefine == 'EarlyDev-LHB' or TargetToDefine == 'EarlyDevelopment-LHB' or TargetToDefine == 'Early Development-LHB':
...
#8/12/19 htc added -RHB and -LHB to practice mode targets.  Probably will never be on existing "River" targets, but added just in case, MIGHT look OK???
    if TargetToDefine == 'SMV1-PRACTICE' or TargetToDefine == 'Practice' or TargetToDefine == 'Pract' or TargetToDefine == 'Target Outline' or TargetToDefine == 'TargetOutline' or \
       TargetToDefine == 'SMV1-PRACTICE-RHB' or TargetToDefine == 'Practice-RHB' or TargetToDefine == 'Pract-RHB' or TargetToDefine == 'Target Outline-RHB' or TargetToDefine == 'TargetOutline-RHB' or \
       TargetToDefine == 'SMV1-PRACTICE-LHB' or TargetToDefine == 'Practice-LHB' or TargetToDefine == 'Pract-LHB' or TargetToDefine == 'Target Outline-LHB' or TargetToDefine == 'TargetOutline-LHB':
    ...
    if TargetToDefine == 'SMV1-PRACT-R' or TargetToDefine == 'PracticeRiver' or TargetToDefine == 'PractRiver' or TargetToDefine == 'Target Outline with River' or TargetToDefine == 'TargetOutlineRiver' or \
       TargetToDefine == 'SMV1-PRACT-R-RHB' or TargetToDefine == 'PracticeRiver-RHB' or TargetToDefine == 'PractRiver-RHB' or TargetToDefine == 'Target Outline with River-RHB' or TargetToDefine == 'TargetOutlineRiver-RHB' or \
       TargetToDefine == 'SMV1-PRACT-R-LHB' or TargetToDefine == 'PracticeRiver-LHB' or TargetToDefine == 'PractRiver-LHB' or TargetToDefine == 'Target Outline with River-LHB' or TargetToDefine == 'TargetOutlineRiver-LHB':
...
#8/12/19 htc added to test routine -LHB and -RHB
    BuildSelectedBaseTargetArrayForThisRun('SMV1-PRACTICE-RHB', '-', 0, '-', 0)
    MarqueeWord('PRACTICE', '')
    Write1260ArrayToArtnetFile()
    ArtdmxToNDB()
    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', 'SMV1-PRACTICE.jpg', width=300)
    ...
    BuildSelectedBaseTargetArrayForThisRun('SMV1-PRACT-R-RHB', '-', 0, '-', 0)
    MarqueeWord('PM RIVER', '')
    Write1260ArrayToArtnetFile()
    ArtdmxToNDB()
    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', 'SMV1-PRACT-R.jpg', width=300)
    ...
    BuildSelectedBaseTargetArrayForThisRun('SMV1-PRACTICE-LHB', '-', 0, '-', 0)
    MarqueeWord('PRACTICE', '')
    Write1260ArrayToArtnetFile()
    ArtdmxToNDB()
    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', 'SMV1-PRACTICE.jpg', width=300)
    ...
    BuildSelectedBaseTargetArrayForThisRun('SMV1-PRACT-R-LHB', '-', 0, '-', 0)
    MarqueeWord('PM RIVER', '')
    Write1260ArrayToArtnetFile()
    ArtdmxToNDB()
    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', 'SMV1-PRACT-R.jpg', width=300)
...
######################  19/27/19 htc - commented out trying to fix "+2+2" bug    impactXnum = impactXnum + 2
...
# 8/12/19 htc - kept this hard coded for now, but changed to "codes" that can be found in the BallSpeedMethodTable.
...
#    strPitchReleaseSpeedMethod = 'measured plate speed plus addon MANUALLY entered on User Record of ' + str(ThisSess_ReleaseSpeedAddon)
    ...
    strPitchPlateSpeedMethod = 'PS-01'
    strPitchReleaseSpeedMethod = 'RS-01: +' + str(ThisSess_ReleaseSpeedAddon)
...
# 8/12/19 htc added fields here because easier to insert in middle of contuation lines.
    SQLstring = SQLstring + 'PmTargetHitAreaX1str, PmTargetHitAreaX1int, PmTargetHitAreaY1int, '

    SQLstring = SQLstring + 'PmTargetHitAreaX2str, PmTargetHitAreaX2int, PmTargetHitAreaY2int, '

    SQLstring = SQLstring + 'PractModeFrameSeqNo, '
...
#10/07/19 htc - force sound ON or OFF with the next two lines.	
    if CurrPitch_Call != 'SUCCESS':  # this line Active if you WANT SOUND, the line below ALWAYS SKIPS Sound logic.
 #####   if CurrPitch_Call != CurrPitch_Call:   #9/30/19 htc forced skip of sound for now - too long of delay in real use (tim Heckler - HIT Indoor)
...
#                                 ***********************
# 08/12/19 htc "forced" curr pitch to marquee content for now, not sure why it's not getting actual value from the "frame" record/data.
    VALstring = VALstring + '"' + CurrPitch_MarqueeContent + '", "' + '-*-' + '", "' + CurrPitch_MovementInfo + '", "' + '01/01/1900' + '", '
...
#8/12/19 htc added fields here because easier to insert in middle of contuation lines.
    VALstring = VALstring + '"' + str(CurrentTargetHitAreaX1str) + '", "' + str(CurrentTargetHitAreaX1int) + '", "' + str(CurrentTargetHitAreaY1int) +  '", '

    VALstring = VALstring + '"' + str(CurrentTargetHitAreaX2str) + '", "' + str(CurrentTargetHitAreaX2int) + '", "' + str(CurrentTargetHitAreaY2int) +  '", '

    VALstring = VALstring + '"' + str(CurrFrame_FrameNumber)  +  '", '
...
#9/19/19 htc - added fiellds to help debug time-out issues users are having per TRF.
    ...
    WriteLogFile('BEFORE while loop--  ExpireTime: [' + str(ExpireTime) + ']', '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('BEFORE while loop--  CurrentTime: [' + str(CurrentTime) + ']', '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('Loope Logic is: while (ExpireTime > CurrentTime)', '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('Value passeed in TimeOutValueToAbort is added to current time: [' + str(TimeOutValueToAbort) + ']', '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
...
#               os.system('sudo rm ' + StopFilePath1)
            if os.path.isfile(StopFilePath2):
                os.system('sudo chmod 777 ' + StopFilePath2)
                os.system('sudo rm ' + StopFilePath2)
            return (4)  # STOP.TXT file found, end program accordingly.
 ...
 #           WriteLogFile('Got this speed (integer): [' + str(intSpeed) + ']', '0274-WaitForSpeedFile(TimeOutValueToAbort)   -')
            ...
            WriteLogFile('Got this speed string: [' + str(SpeedString) + ']', '0274-WaitForSpeedFile(TimeOutValueToAbort)   -')
...
# 9/19/19 htc - added fiellds to help debug time-out issues users are having per TRF.
    WriteLogFile('BEFORE while loop--  StartTime: [' + str(StartTime) + ']',
                     '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('BEFORE while loop--  ExpireTime: [' + str(ExpireTime) + ']',
                     '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('BEFORE while loop--  CurrentTime: [' + str(CurrentTime) + ']',
                     '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile('Loope Logic is: while (ExpireTime > CurrentTime)',
                     '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
    WriteLogFile(
            'Value passeed in TimeOutValueToAbort is added to current time: [' + str(TimeOutValueToAbort) + ']',
            '0271-WaitForSpeedFile(TimeOutValueToAbort)   -')
...
#08/12/19 htc added hit area global values here.
    global CurrentTargetHitAreaX1str
    global CurrentTargetHitAreaX1int
    global CurrentTargetHitAreaY1int
    global CurrentTargetHitAreaX2str
...
#9/19/19 htc
#9/20/19 htc this line threw an error with row[7] = null or none, fix later
#     WriteLogFile('Practice Mode GUID from SESSION RECORD: [' + row[7]  +  ']  ','RunStandardAndPracticeSession-')
...
# 08/22/19 htc - discovered bug in this code, using "UserRow" (user read on user account ID) below instead of "Row" (current session record)

    if str(row[26]) >= '1' and str(row[26]) <= '9':
        ThisSess_ReleaseSpeedAddon = row[26]
    else:
        ThisSess_ReleaseSpeedAddon = 0
#12/04/18 htc - PATCH to Kris code, he's NOT putting speedAddon in SESSION record, I have to get it from student record.
    ...
    if ThisSess_ReleaseSpeedAddon == 0:
        if str(UserRow[35]) >= '1' and str(UserRow[35]) <= '9':
...
#CurrPitch_ExpectedPitch
    CurrPitch_ExpectedPitch = 'STANDARD'       #DEFAULT VALUE
...
#9/19/19 htc  added Practice Mode "normal end" (Counter limit reached).     ThisSess_ShutdownMode
                            ThisSess_ShutdownMode = 'FINISHED'
                            WriteLogFile('Pract Mode REPEAT COUNT LIMIT HIT-: [' + ThisSess_ShutdownMode + '] ','RunStandardAndPracticeSession       -')
...
#10/27/19 htc - modifed pctcoords sub-routine to minus 2 on each axis.
                            strXcoord = XevalNetLocPctCoordsV1(intPctOfXcol)
 #10/27/19 htc - try to fix +2+2 bug
                            TempXintHTC = strX2int(strXcoord)
                            if TempXintHTC >= 3:
                                TempXintHTC = TempXintHTC -2
                                strXcoord = intX2str(TempXintHTC)
                            gintFinalImpactSMgridX = strXcoord
...
# 10/27/19 htc - attempt to fix "+2+2" error.
                            ...
                            if intYcoord > 0 and intYcoord < 30:
                                intYcoord = intYcoord +1

                            gintFinalImpactSMgridY = intYcoord
                ...
                if NoPitchFlag or strXcoord == 101 or strXcoord == 102 or  strXcoord < 'A' or strXcoord > 'Z' or intYcoord < 1 or intYcoord > 26:
#10/3/19 htc - added "fix" to above line - check strXcoord 102 or 101- means no results from Ozan's Impact program.
 ...
 ##                               WriteLogFile('HOME RUN - HOME RUN - HOME RUN', 'Added debug line 1/3/19 htc     ')
  #                              time.sleep(4.9)
            ...
            if ReturnSpeedValue == 3:
                WriteLogFile('SpeedReturnValue = 3, stop.txt file found, ending program normally- reason: [' + ThisSess_ShutdownMode + '] File found at: ' +str(StopFilePath1),
                             '0030-RunStandardAndPracticeSession       -')
                KeepProcessingPitches = False
   
            if ReturnSpeedValue == 4:
                WriteLogFile('SpeedReturnValue = 4, stop.txt file found, ending program normally- reason: [' + ThisSess_ShutdownMode + '] File found at: ' +str(StopFilePath2),
                             '0030-RunStandardAndPracticeSession       -')
                KeepProcessingPitches = False
...
# 9/9/19 htc added software version and MachineID to Marquee display for a few seconds on EVERY BOOT-UP.
        ClearTargetArea()
        ClearMarqueeArea()
        MarqueeWord(smMainVersion, '')
        Write1260ArrayToArtnetFile()  # Write array to NdbArtNetCtrlString.txt file
        ArtdmxToNDB()  # play current NdbArtNetCtrlString.txt on disk in home folder to NDB/LED 10.0.0.100
        time.sleep(1)
