...
smMainVersion = '__SmMain_Ver6b-190102_htc.py'
...
def PickOffPair(strInput, intThisPairNo):
    if intThisPairNo < 1 or intThisPairNo > 4:
        return('error 323')
...
# 12/01/18 htc added flag below to "pair" up "pair's".
    ...
    if intC1_CoordCountComma < 2 or intC1_CoordCountComma > 4:
        return('error-11')
    if intC2_CoordCountComma < 2 or intC2_CoordCountComma > 4:
        return('error-21')
    if intC1_CoordCountSlash < 1 or intC1_CoordCountComma > 4:
        return('error-12')
    if intC2_CoordCountSlash < 1 or intC2_CoordCountComma > 4:
        return('error-22')
...
#    HTC = CurrPitch_ImpactYint
 ...
 #############################################################################################   intermedCoordsB = str(htcNetXinches) + ',-0.1,' + str(htcNetYinches)
    ...
    NetParams = str(htcNetXinches) + ',0,' + str(htcNetYinches)
...
#1/3/19 htc added debug lines (TRF at ABCA in Dallas today, tomorrow (Fri) and Saturday.
    WriteLogFile('First Graph Call COMMAND STRING: ' + str(CmdToCall) + ']','5050-CalculateMovementVer1          -')
...
#    CommandToCall = CmdToCall + cmdParameters1 + Z1Params + '|' + intermedCoordsB + '|' + intermedCoordsA + '|' + NetParams + strAngle + strColor + strTitleMultiLine
    ...
    # 1/3/19 htc added debug lines (TRF at ABCA in Dallas today, tomorrow (Fri) and Saturday.
    WriteLogFile('Second Graph Call COMMAND STRING: ' + str(CmdToCall) + ']', '5050-CalculateMovementVer1          -')
...
#    CommandToCall = CmdToCall + cmdParameters1 + Z1Params + '|' + intermedCoordsB + '|' + intermedCoordsA + '|' +NetParams + strAngle + strColor + strTitleMultiLine
    ...
    # 1/3/19 htc added debug lines (TRF at ABCA in Dallas today, tomorrow (Fri) and Saturday.
    WriteLogFile('Third Graph Call COMMAND STRING: ' + str(CmdToCall) + ']', '5050-CalculateMovementVer1          -')
...
#1/2/19 htc.
                    if gbolHomeRunThisPitch:
                        MarqueeWord('HOME RUN', '')
