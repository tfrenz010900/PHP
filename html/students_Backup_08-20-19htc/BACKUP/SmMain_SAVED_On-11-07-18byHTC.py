...
#
#  BUGS - Misc ToDo's
#  File exists check not working on log file so it doesn't self-truncate/reset, writes NEW file every time!!!
#  Folder/directory separators 'doubled' for windows so I can't do anything in sub-folders or other disk locations.
#  Test Coach/Instructor API calls with James Boe (E.I.)  - as of 7/27/18, no data in smartsheet tables.
#  How to parse/get arguments passed to this program from Brian's PHP code (Session number)
#  Connect to SQL database if on Linux
#  Write log information to file and to database table
#  Play sounds, couldn't get to work on asus laptop 7/25/18 - ALSO - where to play- ond device w/ PHP - I set flag for brian???  Or on NUC?
#  fix value error trapping / reporting / exit routine in PaintThisBox
#  drop the logging from artnetdmx.txt file write to ndb/led routine - extra mess not needed.
#  8/22/18 - read local "system" file and get smartmitt serial number to dispaly on Marquee on bootup.  hard coded now.  htc.
#
# 9/14/18 htc -
#
#
...
import json      #7/26/18 htc
import requests   #8/13/18 htc - started adding calls to EasyInsigh APIs
...
smMainVersion = 'SmMain_Ver3e-181025_HTC.py'
...
#time.sleep(.51)
#9/24/18 htc - clean up impact file if present.
...
#time.sleep(.51)
#9/24/18 htc - always flush out network share
...
gfont_ = '0,5x7,00000,00000,00000,00000,00000,00000,00000'
...
def ArtdmxToNDB():
    ...
    WriteLogFile('Writing NdbArtNetCtrlString.txt to NDB/LED at 10.0.0.100:6454.  ', '0002b-ArtdmxToNDB     -')
...
def Artdmx_Script_Ver1a():
   WriteLogFile('Writing NdbArtNetCtrlString.txt to NDB/LED at 10.0.0.100:6454.  ', '0002a-Artdmx_Script_Ver1a     -')
...
##    StringToWrite = 'Local Time: ' + time.strftime('%a, %d %b %Y %H:%M:%S', time.localtime()) + ' | ' +  PrintThisFunctionName + ' | '  + \
##                    str(MessageToWrite) + ' | TZ:'   + str(time.timezone) + ' |  GMT- ' + time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\n'

## 10/11/18 htc - condensed the date/time strings below to reduce log file size and make more readable.
...
#        goto(1)
    ...
    for intI in range(1, 1261):  # loops 1 to 1260
        if LedArray1260RowNum[intI] >= intStartRow and LedArray1260ColNum[intI] >= intStartCol and LedArray1260RowNum[intI] <= intEndRow and LedArray1260ColNum[intI] <= intEndCol:
            LedArray1260HexColorValue[intI] = strColor6BytesIn


def ClearTargetArea():

    for intI in range(1, 781):  # loops 1 to 780
        LedArray1260HexColorValue[intI] = '000000'


def ClearMarqueeArea():

    for intI in range(781, 1261):  # loops 1 to 780
        LedArray1260HexColorValue[intI] = '000000'


def DisplaySpeed(intSpeed):

    WriteLogFile('display speed routine, (valid speeds 10 to 109), speed: ' + str(intSpeed), '0007-DisplaySpeed                -')
...
#        PaintThisBox(SM_Red_Med, 1, 18, 3, 19)
    ...
    if SpeedDigitOne == '1':
        PaintThisBox(SM_Red_Med, 7, 4, 12, 5)
        PaintThisBox(SM_Red_Med, 9, 6, 12, 17)
        PaintThisBox(SM_Red_Med, 7, 18, 14, 19)

    if SpeedDigitTwo == '1':
        PaintThisBox(SM_Red_Med, 18, 4, 23, 5)
        PaintThisBox(SM_Red_Med, 20, 6, 23, 17)
        PaintThisBox(SM_Red_Med, 18, 18, 25, 19)

    if SpeedDigitOne == '2':
        PaintThisBox(SM_Red_Med, 5, 4, 14, 19)
        PaintThisBox(SM_Off, 5, 6, 11, 10)
        PaintThisBox(SM_Off, 8, 13, 14, 17)

    if SpeedDigitTwo == '2':
        PaintThisBox(SM_Red_Med, 18, 4, 27, 19)
    ...
    if SpeedDigitOne == '7':
        PaintThisBox(SM_Red_Med, 5, 4, 15, 5)
        PaintThisBox(SM_Red_Med, 12, 6, 15, 19)

    if SpeedDigitTwo == '7':
        PaintThisBox(SM_Red_Med, 18, 4, 28, 5)
        PaintThisBox(SM_Red_Med, 25, 6, 28, 19)
    ...
    if SpeedDigitTwo == '8':
        PaintThisBox(SM_Red, 18, 4, 27, 19)
        PaintThisBox(SM_Off, 21, 6, 24, 10)
        PaintThisBox(SM_Off, 21, 13, 24, 17)

    if SpeedDigitOne == '9':
        PaintThisBox(SM_Red_Med, 5, 4, 14, 12)
        PaintThisBox(SM_Off, 8, 6, 11, 10)
        PaintThisBox(SM_Red, 12, 13, 14, 19)

    if SpeedDigitTwo == '9':
        PaintThisBox(SM_Red_Med, 18, 4, 27, 12)
        PaintThisBox(SM_Off, 21, 6, 24, 10)
        PaintThisBox(SM_Red, 25, 13, 27, 19)
...
def RunBootUpLedCheck():
    ...
    for intI in range(1, 50):  # loops 1 to 50
        UpdateSuccessMissCountersInLedArray(intI,intI)
        Write1260ArrayToArtnetFile()
        ArtdmxToNDB()
 #       time.sleep(0.05)
    ...
    for intI in range(1, 21):  # loops 1 to 20
        UpdateNoPitchCountInLedArray(intI)
        Write1260ArrayToArtnetFile()
        ArtdmxToNDB()
#        time.sleep(0.050)
...
def UpdateSuccessMissCountersInLedArray(NumSuccess, NumMiss):
    ...
    if ThisSess_DisplayPitchCountBarsYN == '' or ThisSess_DisplayPitchCountBarsYN == None or ThisSess_DisplayPitchCountBarsYN == 'None' or \
        ...
        WriteLogFile('UpdateSuccessMissCountersInLedArray Success/miss values: [' + str(NumSuccess) + '/' + str(NumMiss) + ']', '0010-UpdateSuccessMissInLedArray-')
...
def UpdateNoPitchCountInLedArray(NoPitchCount):
    ...
    if ThisSess_DisplayPitchCountBarsYN == '' or  str(ThisSess_DisplayPitchCountBarsYN[0]).upper() == 'Y':
        ...
        WriteLogFile('UpdateNoPitchCountInLedArray: [' + str(NoPitchCount) + ']', '0011-UpdateNoPitchCountInLedArray-')
...
def SeqLightNo(Xin, Yin):
    ...
    for intI in range(1, 781):  # loops 1 to 780
        if LedArray1260ColNum[intI] == Xin:
            if LedArray1260RowNum[intI] == Yin:
                return LedArray1260SeqNo[intI]
...
# 10/11/18 htc    TargetNameForThisSession = ''
    ...
    SQLstring = SQLstring + 'ImpactFileLedViewTag, LineImageRightTag, LineImageLeftTag, TargetFileLedViewTag'
    ...
    VALstring = VALstring + '"' + '-*-' + '", "' + '-*-' + '", "' + CurrPitch_MovementInfo + '", "' + '01/01/1900' + '", '
    ...
    VALstring = VALstring + '"' + 'impact LED tag' + '", "' + 'line img right tag' + '", "' + 'line img left tag' + '", "' + 'target img led tag' + '")'
...
# ADD THESE FIELDS
    ...
    #  CurrPitch_FullPath5
    #  CurrPitch_FullPath6
    #UserDisplayImage1FullLocalPath
...
#    WriteLogFile('Attempting to create this file from (NdbArtNetCtrlString.txt) in home folder: [' + PathForFilesToDisplayOnDevice + '/ImpactNEW.jpg' ']', '0071-DisplayImpactLocation              -')

#    if os.path.isfile(PathForFilesToDisplayOnDevice + '/ImpactNEW.jpg'):
#        os.system('sudo rm ' + PathForFilesToDisplayOnDevice + '/ImpactNEW.jpg')

#    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', PathForFilesToDisplayOnDevice + '/ImpactNEW.jpg', width=300)
    ArtnetTxtToJpg('NdbArtNetCtrlString.txt', NetworkSharePath + '/Impact.jpg', width=300)
...
#10/23/18 HTC Added in set vars routine, more options now in pract mode setup on SSheet.   CurrFrame_MoveOnMissCount = 99  #9/29/18 htc - TRF didn't setup any MISS counts in the SmartSheet practice mode detail "FRAME" table, always force to 99 for now.
    ...
    ##################   WAIT FOR SPEED.TXT or TIMEOUT              It says SECONDS but is really minutes from PHP interface!!! 9/23/18/htc
            ...
            WriteLogFile('Response from WaitForSpeedFile:  ReturnSpeedValue = [' + str(ReturnSpeedValue) + ']', '0030-RunStandardAndPracticeSession       -')
    ...
    # 9/16/18 htc - take care of "Cam1_20180913165741.png" files 1 and 2 here. CRITICAL FRAME
                        if EachFile[0:3] == 'Cam' and EachFile[5:13] != 'ballLine' and EachFile[-4:-1] == '.pn':
                            ...
                            if CamNumOnly == '1':
                                ...
                                if os.path.isfile(PathForFilesToDisplayOnDevice + '/Cam1CriticalFrameNEW.png'):
                                    os.system('sudo rm ' + PathForFilesToDisplayOnDevice + '/Cam1CriticalFrameNEW.png')
                                CommandToCall = 'cp ' + NetworkSharePath + '/Cam1CriticalFrame.png ' + PathForFilesToDisplayOnDevice + '/Cam1CriticalFrameNEW.png'
                                CmdCallResult = os.system(CommandToCall)
    ...
    #                            CmdCallResult = os.system(CommandToCall)
                            ...
                            if CamNumOnly == '2':
                                ...
                                if os.path.isfile(PathForFilesToDisplayOnDevice + '/Cam2CriticalFrameNEW.png'):
                                    os.system('sudo rm ' + PathForFilesToDisplayOnDevice + '/Cam2CriticalFrameNEW.png')
                                    CommandToCall = 'cp ' + NetworkSharePath + '/Cam2CriticalFrame.png ' + PathForFilesToDisplayOnDevice + '/Cam2CriticalFrameNEW.png'
    ...
    #9/25/18 htc - added from here down mother-fuckers!!!!!!!!!!!!!!!!!
                    # if the file(s) already exists for some fucked up reason in CURRENT SESSION folder, FUCK IT, delete the mother-fucker and add what you fuckin' got right fuckin' now!!   LOL  :)
                    CurrPitch_FullPath1 = 'fuking error mate!'
                    CurrPitch_FullPath2 = 'fuking error mate!'
                    CurrPitch_FullPath3 = 'fuking error mate!'
    ...
    #9/30/18 htc cutoff front of path to store in format Kris needs to display in pitch table on users device. -- same for all files below.
                    ...
                    if os.path.isfile(NetworkSharePath + '/Cam1BallCenters.txt'):
                        ...
                        CurrPitch_MovementInfo =  Cam1CentersFile.read()
                    ...
                    if os.path.isfile(NetworkSharePath + '/Cam2BallCenters.txt'):
                        ...
                        CurrPitch_MovementInfo = CurrPitch_MovementInfo + '-:-' + Cam2CentersFile.read()
    ...
    #  9/26/18 htc - added support for SIX display windows for the "Current Pitch" on the users devide PITCH STATUS screen.
                    CurrPitch_FullPath4 = 'future use'
                    CurrPitch_FullPath5 = 'future use'
                    CurrPitch_FullPath6 = 'future use'
    ##9/30/18 htc cutoff front of path to store in format Kris needs to display in pitch table on users device. -- same for all files below.
    ##                    CurrPitch_FullPath1 = str(CurrPitch_FullPath1)[23:100]
 ...
 #                   CurrPitch_TargetGUID  =  ThisSess_StandTargetGuid   # TargetToDefine
                    ...
                    CurrPitch_MovementInfo = RemoveCRLF(CurrPitch_MovementInfo)
    ...
    # 9/25/18 htc set above with copy to archive folder section                CurrPitch_FullPath3 = 'IMPACT DOT JPEG FULL PATH'
                    ...
                    CurrPitch_FullPath4 = 'reserved for future use - movement X?'
                    CurrPitch_FullPath5 = 'reserved for future use - movement Y?'
                    CurrPitch_FullPath6 = 'reserved for future use - movement Y?'
